...
# 'AugCD' is the legacy class name of MdaCD
class AugCD(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.fuse_concat_index = fuse_concat_index
    ...
    def _decode_head_forward_train_with_text(self, x, fuse_diff, textA, textB: List[Tensor],
                                            data_samples: SampleList) -> dict:
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        loss_decode = self.decode_head.custom_loss(x, fuse_diff, textA, textB, data_samples, self.train_cfg)
        losses.update(add_prefix(loss_decode, 'decode'))
        return losses
    ...
    def after_extract_feat_clip(self, x, text):
        ...
        torch.Size([20, 1024, 16, 16])
        torch.Size([20, 2048, 8, 8])'''
        x_orig = list(x[0:4])
        global_feat, visual_embeddings = x[4] # torch.Size([20, 1024]) torch.Size([20, 1024, 8, 8])
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        if self.with_identity_head:
            loss_identity_sm = self._identity_head_forward_train(
                fused_embeddings_diff/self.tau, data_samples, 'aux_fused_embeddings')
