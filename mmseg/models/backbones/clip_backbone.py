...
class DF(nn.Module):
    def __init__(self, dim):
        ...
        self.visual_align = nn.Sequential(
            nn.Conv1d(in_channels=65, out_channels=K, kernel_size=1), nn.LayerNorm(dim))
        self.text_layer1 = ConvModule(
            in_channels=dim,
            out_channels=dim,
            kernel_size=1,
            stride=1,
    ...
    def forward(self, vis, text):
        text = text.permute(0, 2, 1) # [12, 256, 9]
        vis = self.visual_align(vis).permute(0, 2, 1) # [12, 256, 9]

        text = self.text_layer1(text)
        text = self.text_layer2(text)
        text = self.text_layer3(text)
        text = text + vis
        text = self.text_layer4(text)
        text = self.text_layer5(text)
        text = self.text_layer6(text)
        text = text + vis
        text = self.text_layer7(text)
        text = self.text_layer8(text)
        text = self.text_layer9(text)
        text = text + vis

        text = text.permute(0, 2, 1) # [12, 9, 256]
        return text
