...
def make_divisible(value, divisor=8):
    res = int((value + divisor / 2) // divisor) * divisor
    return max(res, 1)
class SELayer_1D(nn.Module):
    def __init__(self,
        ...
        self.global_avgpool = nn.AdaptiveAvgPool1d(1)  # 1D pooling
        self.mid_channels = make_divisible(channels // ratio, 8)
    ...
    def forward(self, x):
        ...
        out = self.global_avgpool(x)  # Output shape: (batch_size, channels, 1)
        # print(f'out.shape={out.shape}, x.shape={x.shape}')
        # print(f'self.mid_channels={self.mid_channels}')
        # print("conv1 in_channels:", self.conv1.in_channels)
        # input('')
        out = self.conv1(out)         # Output shape: (batch_size, inter_channels, 1)
        out = self.conv2(out)         # Output shape: (batch_size, channels, 1)
        return x * out  # Output shape: (batch_size, channels, length)
...
class SwinTextDecode(BaseDecodeHead):
    def __init__(self, feature_strides, channels, text_dim=1024, **kwargs):
        ...
        self.minus_conv = ConvModule(in_channels=2, out_channels=2, kernel_size=3, 
                                     stride=1, padding=1, conv_cfg=dict(type='Conv1d'))
        self.channel_att = nn.Sequential(SELayer_1D(16),
                                         nn.Conv1d(in_channels=16, out_channels=2, kernel_size=3, padding=1))
        self.linear = nn.Sequential(nn.Linear(self.text_dim, 128),
                                     nn.LayerNorm(128))
        self.e4conv = SELayer(258, 256)

    def forward(self, inputs, score_diff, textA, textB):
        e1, e2, e3, e4 = inputs
        text_minus = self.minus_conv(torch.abs(textA-textB)) # [20, 9, 128]
        text_diff = F.sigmoid(1-torch.cosine_similarity(textA, textB, dim=2)).unsqueeze(-1).expand(-1, -1, self.text_dim)
        text = torch.cat([textA*text_diff, textB*text_diff, text_minus, text_minus,
                        textA, textB, textA, textB], dim=1)
        text = self.linear(self.channel_att(text))
        e4 = self.e4conv(torch.cat([e4, score_diff], dim=1))
